...
suite('Analyzer', () => {
    ...
    setup(() => {
        analyzer = new analyzer_1.Analyzer({
            urlLoader: new fs_url_loader_1.FSUrlLoader(__dirname),
    ...
    suite('analyze()', () => {
        ...
        test('creates "missing behavior" warnings on imported documents without elements', () => __awaiter(this, void 0, void 0, function* () {
            ...
            const chainedDocument = document.getOnlyAtId('document', 'static/chained-missing-behavior/chained.html');
            ...
            chai_1.assert.deepEqual(document.getWarnings(false), []);
            chai_1.assert.deepEqual(document.getWarnings(true), [expectedWarning]);
            // TODO(#372): this should be false, we should treat inline documents
            //     as not requiring `deep` to be true.
            chai_1.assert.deepEqual(chainedDocument.getWarnings(true), [expectedWarning]);
        }));
        test('an inline document can find features from its container document', () => __awaiter(this, void 0, void 0, function* () {
            ...
            chai_1.assert.equal(documents.size, 4);
            const inlineDocuments = Array.from(document.getFeatures(false))
            ...
            const subBehavior = behaviorJsDocument.getOnlyAtId('behavior', 'MyNamespace.SubBehavior');
        ...
        test('an inline script can find features from its container document', () => __awaiter(this, void 0, void 0, function* () {
            ...
            const subBehavior = inlineJsDocument.getOnlyAtId('behavior', 'TestBehavior');
        ...
        test('an external script can find features from its container document', () => __awaiter(this, void 0, void 0, function* () {
            ...
            // The inline document can find the container's imported features
            const subBehavior = scriptDocument.getOnlyAtId('behavior', 'TestBehavior');
            chai_1.assert.equal(subBehavior.className, 'TestBehavior');
        }));
        ...
        test('an inline document can find behaviors from its container document', () => __awaiter(this, void 0, void 0, function* () {
            ...
            // subbehavior.html, and their inline js documents
            const documents = document.getByKind('document');
            chai_1.assert.equal(documents.size, 6);
            const inlineDocuments = Array.from(document.getFeatures(false))
                .filter((d) => d instanceof model_1.Document && d.isInline);
            chai_1.assert.equal(inlineDocuments.length, 1);
            // This is the main purpose of the test: get a feature from the inline
        ...
        test('returns a Document with warnings for malformed files', () => __awaiter(this, void 0, void 0, function* () {
            ...
            chai_1.assert(document.getWarnings().length >= 1);
        }));
        test('analyzes transitive dependencies', () => __awaiter(this, void 0, void 0, function* () {
            ...
            chai_1.assert.deepEqual(Array.from(root.getByKind('document'))
            ...
            chai_1.assert.deepEqual(Array.from(root.getByKind('import')).map((d) => d.url), [
            ...
            const inlineOnly = root.getOnlyAtId('document', 'static/dependencies/inline-only.html');
            chai_1.assert.deepEqual(Array.from(inlineOnly.getByKind('document'))
                .map((d) => d.parsedDocument.type), ['html', 'js', 'css']);
            const leaf = root.getOnlyAtId('document', 'static/dependencies/leaf.html');
            chai_1.assert.deepEqual(Array.from(leaf.getByKind('document')), [leaf]);
            const inlineAndImports = root.getOnlyAtId('document', 'static/dependencies/inline-and-imports.html');
            chai_1.assert.deepEqual(Array.from(inlineAndImports.getByKind('document'))
                .map((d) => d.parsedDocument.type), ['html', 'js', 'html', 'html', 'css']);
            const inFolder = root.getOnlyAtId('document', 'static/dependencies/subfolder/in-folder.html');
            chai_1.assert.deepEqual(Array.from(inFolder.getByKind('document')).map(d => d.url), [
                'static/dependencies/subfolder/in-folder.html',
                'static/dependencies/subfolder/subfolder-sibling.html'
            ]);
            // check de-duplication
            chai_1.assert.equal(inlineAndImports.getOnlyAtId('document', 'static/dependencies/subfolder/in-folder.html'), inFolder);
        }));
        test(`rejects for files that don't exist`, () => __awaiter(this, void 0, void 0, function* () {
            yield test_utils_1.invertPromise(analyzer.analyze('/static/does_not_exist'));
        }));
    ...
    suite('_parse()', () => {
        test('loads and parses an HTML document', () => __awaiter(this, void 0, void 0, function* () {
            const doc = yield analyzer['_cacheContext']['_parse']('static/html-parse-target.html');
        ...
        test('loads and parses a JavaScript document', () => __awaiter(this, void 0, void 0, function* () {
            const doc = yield analyzer['_cacheContext']['_parse']('static/js-elements.js');
        ...
        test('returns a Promise that rejects for non-existant files', () => __awaiter(this, void 0, void 0, function* () {
            yield test_utils_1.invertPromise(analyzer['_cacheContext']['_parse']('static/not-found'));
        }));
    });
    suite('_getScannedFeatures()', () => {
        test('default import scanners', () => __awaiter(this, void 0, void 0, function* () {
          ...
          <link rel="stylesheet" href="foo.css"></link>
            ...
            const features = (yield analyzer['_cacheContext']['_getScannedFeatures'](document));
    ...
    suite('analyzePackage', () => {
        test('produces a project with the right documents', () => __awaiter(this, void 0, void 0, function* () {
            const analyzer = new analyzer_1.Analyzer({
                urlLoader: new fs_url_loader_1.FSUrlLoader(path.join(__dirname, 'static', 'project'))
            });
            const project = yield analyzer.analyzePackage();
            // The root documents of the project are a minimal set of documents whose
            // imports touch every document in the project.
            chai_1.assert.deepEqual(Array.from(project['_documents']).map(d => d.url).sort(), ['cyclic-a.html', 'root.html', 'subdir/root-in-subdir.html']
                .sort());
            ...
            // project.
            chai_1.assert.deepEqual(Array.from(project.getByKind('element')).map(e => e.tagName).sort(), [
                'root-root',
                'leaf-leaf',
                'cyclic-a',
                'cyclic-b',
...
`));
            ...
            test('analyzes multiple imports of the same behavior', () => __awaiter(this, void 0, void 0, function* () {
                ...
                chai_1.assert.deepEqual(documentA.getWarnings(true), []);
                chai_1.assert.deepEqual(documentB.getWarnings(true), []);
            }));
