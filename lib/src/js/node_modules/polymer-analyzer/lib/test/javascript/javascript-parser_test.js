...
suite('JavaScriptParser', () => {
    let parser;
    setup(() => {
        parser = new javascript_parser_1.JavaScriptParser({ sourceType: 'script' });
    });
    suite('parse()', () => {
        test('parses classes', () => {
            let contents = `
      ...
      `;
        ...
        test('throws syntax errors', () => {
            let file = fs.readFileSync(path.resolve(__dirname, '../static/js-parse-error.js'), 'utf8');
            chai_1.assert.throws(() => parser.parse(file, '/static/js-parse-error.js'));
        });
        test('attaches comments', () => {
            let file = fs.readFileSync(path.resolve(__dirname, '../static/js-elements.js'), 'utf8');
