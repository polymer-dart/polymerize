    ...
    suite('scan()', () => {
      ...
      Polymer({
        ...
        listeners: []
            ...
            chai_1.assert.equal(features[0].warnings.filter(w => w.code === 'invalid-listeners-declaration').length, 0);
            // Emit warning for non-object `listeners` literal
            chai_1.assert.equal(features[1].warnings.filter(w => w.code === 'invalid-listeners-declaration').length, 1);
        }));
    });
});
