            ...
            const myVisitor = (node) => {
                if (isJsScriptNode(node)) {
                    ...
                    if (src) {
                        ...
                        features.push(new html_script_tag_1.ScannedScriptTagImport('html-script', importUrl, document.sourceRangeForNode(node), document.sourceRangeForAttributeValue(node, 'src'), node));
                    }
                    else {
